...
public class InsertTemplate implements RequestHandler {
    ...
    private String insertTemplate(Bitmap old_bmp, User user) {
        //转RGB
        //  byte[] mBGR = FileUtils.bitmapToBGR24(bmp);
        //生成随机图片ID
        Bitmap bmp =CameraHelp.alignBitmapForNv21(old_bmp);//裁剪
       ...
       // bitmapToNv21
        ...
        if ((MyApplication.mFaceLibCore.FaceDetection(nv21, w, h, result) == 0)&&(result.size() > 0)) {
            ...
            if (ret == 0) {
                ...
                CameraHelp.saveImgToDisk(ImagePath, imageID + ".jpg", old_bmp);
